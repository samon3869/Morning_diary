...
def contains_uppercase_letters(value):
    for char in value:
        if char.isupper():
            return True
    return False
...
def contains_lowercase_letters(value):
    for char in value:
        if char.islower():
            return True
    return False
...
class CustomPasswordValidator:
    def validate(self, password, user=None):
        if(
            ...
            not contains_uppercase_letters(password) or
            not contains_lowercase_letters(password) or
            not contain_number(password)
        ):
